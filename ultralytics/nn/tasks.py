...
class BaseModel(nn.Module):
    ...
    def forward(self, x, epoch=0, *args, **kwargs):
        """
        ...
        if isinstance(x, dict):  # for cases of training and validating while training.
            return self.loss(x, epoch=epoch, *args, **kwargs)
    ...
    def loss(self, batch, preds=None, epoch=0):
        ...
        return self.criterion(preds, batch, epoch=epoch)
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, DynamicHead,DynamicPose)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            s = 640  # 2x min stride
            ...
            def _forward(x):
                ...
                return self.forward(x)[0] if isinstance(m, (Segment, Pose, OBB, DynamicPose, )) else self.forward(x)
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def loss(self, batch, epoch, preds=None):
        """
        ...
        loss = self.criterion(
            (dec_bboxes, dec_scores), targets, epoch=epoch, dn_bboxes=dn_bboxes, dn_scores=dn_scores, dn_meta=dn_meta
        )
...
def torch_safe_load(weight, safe_only=False):
    ...
    ckpt, file = torch_safe_load("path/to/PPF-YOLO.pt", safe_only=True)
    ```
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3, PSA, SCDown, C2fCIB,C2PSA_MLCA, C3k2_iAFF,}:
            c1, c2 = ch[f], args[0]
            ...
            args = [c1, c2, *args[1:]]
            if m in {
                BottleneckCSP, C1, C2, C2f, C3k2, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2fPSA, C2fCIB, C2PSA,C3k2_iAFF,}:
        ...
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect, DynamicPose,}:
        ...
        if isinstance(c2, list) and not goldyolo and not twoBackbone:
            m_ = m
            m_.backbone = True
        else:
...
def guess_model_task(model):
    ...
    def cfg2task(cfg):
        ...
        if m in ("segment", ''):
            return "segment"
        if m in ("pose", "dynamicdcnv3pose", "dynamicpose", "fasffpose"):
            return "pose"
        if m == "obb":
            return "obb"
        else:
    ...
    if isinstance(model, nn.Module):  # PyTorch modeldsdconvsegment
        ...
        for m in model.modules():
            if isinstance(m, (Segment, )):
                ...
                return "classify"
            elif isinstance(m, Pose, DynamicPose, ):
                return "pose"
            elif isinstance(m, OBB):
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, v10Detect,  DynamicHead,
                                )):
                return "detect"
